...
```
./setup_f5_gke.sh -c lw-sales-trng -p lw-sales -z us-west1 --num-solr 1 -n wtsenglabs -r wtsenglabs \
  -t -h wtsenglabs.lucidworkssales.com \
  --version 5.0.2
```
...
Use `wtsenglabs.lucidworkssales.com` for the hostname as we've preconfigured the Cloud DNS zones for these domain names.
...
api-gateway:
  ...
  ingress:
    enabled: true
    host: "wtsenglabs.lucidworkssales.com"
    path: "/*"
    tls:
      enabled: true
    annotations:
...
```
curl -u admin:<PASSWORD> -XPOST https://wtsenglabs.lucidworkssales.com/oauth2/token
```
