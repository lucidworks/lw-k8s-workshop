...
```
./setup_f5_gke.sh -c proserve-trng -p proserve -z us-central1 --num-solr 1 -n ashum -r ashum \
  -t -h ashum.lucidworksproserve.com \
  --version 5.0.2
```
...
Use `ashum.lucidworksproserve.com` for the hostname as we've preconfigured the Cloud DNS zones for these domain names.
...
api-gateway:
  ...
  ingress:
    enabled: true
    host: "ashum.lucidworksproserve.com"
    path: "/*"
    tls:
      enabled: true
    annotations:
...
```
curl -u admin:<PASSWORD> -XPOST https://ashum.lucidworksproserve.com/oauth2/token
```
