...
```
./setup_f5_gke.sh -c lw-sales-trng -p lw-sales -z us-west1 --num-solr 1 -n tjp -r tjp \
  -t -h tjp.lucidworkssales.com \
  --version 5.0.2
```
...
Use `tjp.lucidworkssales.com` or `tjp.lucidworksproserve.com` for the hostname as we've preconfigured the Cloud DNS zones for these domain names.
...
```
curl -u admin:<PASSWORD> -XPOST https://tjp.lucidworkssales.com/oauth2/token
```
