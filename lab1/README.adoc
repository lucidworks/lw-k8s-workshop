...
```
./setup_f5_gke.sh -c lw-sales-trng -p lw-sales -z us-west1 -n shs4 --version 5.0.2
```
...
```
kubectl --namespace shs4 get service proxy -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'
```
The port is `6764`
...
The setup script created a new shell script named `gke_lw-sales-trng_shs4_fusion_upgrade.sh` in the `fusion-cloud-native` directory.
