...
```
./setup_f5_gke.sh -c lw-sales-trng -p lw-sales -z us-west1 --num-solr 1 -n bhb -r bhb \
  -t -h bhb.lucidworkssales.com \
  --version 5.0.2
```
...
Use `bhb.lucidworkssales.com` for the hostname as we've preconfigured the Cloud DNS zones for these domain names.
...
```
curl -u admin:<PASSWORD> -XPOST https://bhb.lucidworkssales.com/oauth2/token
```
