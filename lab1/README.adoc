...
```
./setup_f5_gke.sh -c lw-sales-trng -p lw-sales -z us-west1 --num-solr 1 -n wtlabs -r wtlabs \
  -t -h wtlabs.lucidworkssales.com \
  --version 5.0.2
```
...
Use `wtlabs.lucidworkssales.com` for the hostname as we've preconfigured the Cloud DNS zones for these domain names.
...
api-gateway:
  ...
  ingress:
    enabled: true
    host: "wtlabs.lucidworkssales.com"
    path: "/*"
    tls:
      enabled: true
    annotations:
...
```
curl -u admin:<PASSWORD> -XPOST https://wtlabs.lucidworkssales.com/oauth2/token
```
