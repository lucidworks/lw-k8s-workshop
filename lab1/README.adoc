...
```
./setup_f5_gke.sh -c proserve-trng -p proserve -z us-west1 -n shs2 --version 5.0.2
```
...
```
kubectl --namespace shs2 get service proxy -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'
```
The port is `6764`
