...
```
kubectl expose deployment efordelon-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm efordelon-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:efordelon -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n efordelon
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/proserve/gatling-qps:efordelon \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://efordelon.lucidworksproserve.com -Dqps.app=lab4" \
...
```
k cp efordelon-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
