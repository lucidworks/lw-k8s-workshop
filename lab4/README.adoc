...
```
kubectl expose deployment emaq-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm emaq-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:emaq -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n emaq
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/proserve/gatling-qps:emaq \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://emaq.lucidworksproserve.com -Dqps.app=lab4" \
...
```
k cp emaq-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
