...
```
kubectl expose deployment tjp3-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm tjp3-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:tjp3 -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n tjp3
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/proserve/gatling-qps:tjp3 \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://tjp3.lucidworksproserve.com -Dqps.app=lab4" \
...
```
k cp tjp3-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
