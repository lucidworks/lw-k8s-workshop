...
```
kubectl expose deployment wtlabs-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm wtlabs-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:wtlabs -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n wtlabs
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/lw-sales/gatling-qps:wtlabs \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://wtlabs.lucidworkssales.com -Dqps.app=lab4" \
...
```
k cp wtlabs-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
