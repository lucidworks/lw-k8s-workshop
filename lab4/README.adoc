...
```
kubectl expose deployment mikeche-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm mikeche-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:mikeche -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n mikeche
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/lw-sales/gatling-qps:mikeche \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://mikeche.lucidworkssales.com -Dqps.app=lab4" \
...
```
k cp mikeche-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
