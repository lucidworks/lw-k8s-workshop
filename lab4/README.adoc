...
```
kubectl expose deployment bhb-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm bhb-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:bhb -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n bhb
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/lw-sales/gatling-qps:bhb \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://bhb.lucidworkssales.com -Dqps.app=lab4" \
...
```
k cp bhb-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
