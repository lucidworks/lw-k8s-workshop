...
```
kubectl expose deployment mikechesnut-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm mikechesnut-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:mikechesnut -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n mikechesnut
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/lw-sales/gatling-qps:mikechesnut \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://mikechesnut.lucidworkssales.com -Dqps.app=lab4" \
...
```
k cp mikechesnut-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
