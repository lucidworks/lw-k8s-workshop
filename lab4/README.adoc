...
```
kubectl expose deployment npe-train-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm npe-train-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:npe-train -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n npe-train
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/proserve/gatling-qps:npe-train \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://npe-train.lucidworksproserve.com -Dqps.app=lab4" \
...
```
k cp npe-train-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
