...
```
kubectl expose deployment shs2-graf-grafana --type=LoadBalancer --name=grafana
```
...
```
k get cm shs2-query-pipeline-warming-queries -o yaml --export
```
...
```
docker build -t gatling-qps:shs2 -f docker/Dockerfile .
```

Run a script to tag and push the image from local to GCP:
```
./gcp_docker.sh -p <PROJECT> -n shs2
```
...
kubectl run --generator=run-pod/v1 --restart=Never \
   --image=us.gcr.io/proserve/gatling-qps:shs2 \
   --env="JAVA_OPTS=-Dqps.fusion.url=https://shs2.lucidworksproserve.com -Dqps.app=lab4" \
...
```
k cp shs2-solr-search-0:/var/solr/logs/solr_gc.log ./solr_gc.log
```
