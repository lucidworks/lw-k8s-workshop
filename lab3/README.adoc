...
```
kubectl exec -it mikeche-solr-0 -- /bin/bash
```
...
```
k get pods -o wide | grep mikeche-solr | grep -v exporter | sed 's/   */:/g' | cut -d : -f 7 | sort | uniq | wc -l
```
Should be `7` ... why?
...
Open a port-forward to one of your Solr pods: `k port-forward mikeche-solr-0 8983`
...
k exec dnsutils nslookup mikeche-solr-search-headless
```
