...
```
kubectl exec -it npe-train-solr-0 -- /bin/bash
```
...
```
k get pods -o wide | grep npe-train-solr | grep -v exporter | sed 's/   */:/g' | cut -d : -f 7 | sort | uniq | wc -l
```
Should be `7` ... why?
...
Open a port-forward to one of your Solr pods: `k port-forward npe-train-solr-0 8983`
...
k exec dnsutils nslookup npe-train-solr-search-headless
```
