...
Throughout this lab, replace `bhb` with the release you used in lab1 (-r) and `bhb.lucidworkssales.com` with the hostname you provided in lab1 (-h).
...
Use kubectl to scale the `bhb-query-pipeline` deployment up and down

Up:
```
k scale deployment bhb-query-pipeline --replicas=2
```
...
Down:
```
k scale deployment bhb-query-pipeline --replicas=1
```
...
```
curl -u admin:<PASSWORD> "https://bhb.lucidworkssales.com/api/apps/lab1/query/lab1?q=*:*"
```

Did you know every Fusion microservice exposes Swagger API documentation? Try loading this URL in your browser:

https://bhb.lucidworkssales.com/query/swagger-ui.html
