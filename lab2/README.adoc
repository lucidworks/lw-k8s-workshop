...
=== Step 8: Apply Config Changes & Upgrade
...
Take a moment to find this upgrade script and inspect its contents.
...
After making your change to the custom values yaml, run the upgrade script to apply these changes.

Watch the updated `api-gateway` pods as the upgrade rolls out across the cluster. `kubectl get pods --watch`.

=== Step 9: Get a JWT from the Gateway

Lastly, let's request a JWT from the Gateway and decode it to get a feel for how JWT's work in Fusion 5.
