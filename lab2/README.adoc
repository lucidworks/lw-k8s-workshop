...
Throughout this lab, replace `emaq` with the release you used in lab1 (-r) and `emaq.lucidworksproserve.com` with the hostname you provided in lab1 (-h).
...
Use kubectl to scale the `emaq-query-pipeline` deployment up and down

Up:
```
k scale deployment emaq-query-pipeline --replicas=2
```
...
Down:
```
k scale deployment emaq-query-pipeline --replicas=1
```
...
```
curl -u admin:<PASSWORD> "https://emaq.lucidworksproserve.com/api/apps/lab1/query/lab1?q=*:*"
```

Did you know every Fusion microservice exposes Swagger API documentation? Try loading this URL in your browser:

https://emaq.lucidworksproserve.com/query/swagger-ui.html
