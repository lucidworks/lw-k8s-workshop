...
Throughout this lab, replace `shs2` with the release you used in lab1 (-r) and `shs2.lucidworksproserve.com` with the hostname you provided in lab1 (-h).
...
Use kubectl to scale the `shs2-query-pipeline` deployment up and down

Up:
```
k scale deployment shs2-query-pipeline --replicas=2
```
...
Down:
```
k scale deployment shs2-query-pipeline --replicas=1
```
...
```
curl -u admin:<PASSWORD> "https://shs2.lucidworksproserve.com/api/apps/lab1/query/lab1?q=*:*"
```

Did you know every Fusion microservice exposes Swagger API documentation? Try loading this URL in your browser:

https://shs2.lucidworksproserve.com/query/swagger-ui.html
