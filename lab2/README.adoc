...
Use kubectl to scale the `efordelon-query-pipeline` deployment up and down

Up:
```
k scale deployment efordelon-query-pipeline --replicas=2
```
...
Down:
```
k scale deployment efordelon-query-pipeline --replicas=1
```
...
https://efordelon.lucidworksproserve.com/query/swagger-ui.html
